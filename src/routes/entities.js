...
router.get('/towers', async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(`
      SELECT t.id, t.name, t.address, t.created_by, t.created_at, t.updated_at,
...
router.patch('/towers/:id', async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      ...
      [name, address, image_url, req.params.id]
    );
...
router.patch('/companies/:id', async (req, res) => {
  try {
    ...
    res.json({ company: await resolveImageRow(rows[0]) });
  } catch (err) {
    console.error(err);
...
router.get('/organizations', async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(`
      SELECT o.id, o.name, o.address, o.created_by, o.created_at, o.updated_at,
...
router.patch('/locations/:id', async (req, res) => {
  try {
    ...
    res.json({ location: await resolveImageRow(rows[0]) });
  } catch (err) {
    console.error(err);
